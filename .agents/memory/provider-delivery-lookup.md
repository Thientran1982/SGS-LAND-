...
**How to apply:** Keep the tenant/date/recipient delivery key stable, update the claim only after a definitive lookup, and treat unsupported provider capabilities as unknown rather than as “not received.” Automatic recovery must replay one recipient at a time from the saved report snapshot, with a bounded audit trail and dead-letter alert.