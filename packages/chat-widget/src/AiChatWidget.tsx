...
export interface AiChatWidgetProps {
  /**
   * Giao thuc chat. Mac dinh: landing-ai (Gemini one-shot, POST /api/landing-ai/consult).
   * App CRM co the truyen createMinhTransport() de noi thang toi agent Minh
   * ma khong phai fork lai UI.
   */
  transport?: ChatTransport;
  /** Base URL backend (rong = same-origin qua proxy dev). */
  apiBase?: string;
}

export function AiChatWidget({ transport, apiBase }: AiChatWidgetProps = {}) {
  const transportRef = useRef<ChatTransport | null>(null);
  if (!transportRef.current) {
    transportRef.current = transport || createLandingAiTransport({ apiBase });
  }
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const { reply } = await transportRef.current.send({
        text,
        history: messages,
      });
      ...
      const aiMsg: Message = {
        ...
        role: "assistant",
        content: reply || "Xin lỗi, tôi chưa hiểu câu hỏi của bạn. Bạn có thể hỏi lại không?",
        ts: Date.now(),
      };
      ...
      console.error("[AiChatWidget] transport failed:", transportRef.current?.name, err);
      setMessages((prev) => [
        ...prev,
        {
          id: (Date.now() + 1).toString(),
