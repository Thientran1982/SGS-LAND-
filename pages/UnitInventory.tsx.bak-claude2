 ...
 * Bổ sung tồn kho chi tiết từng căn trên nền ma trận giá (project_price_matrix).
 */
...
// Sinh dữ liệu tồn kho mẫu từ ma trận giá cơ sở theo tower + tầng.
function buildUnits(): Unit[] {
  const basePriceByTower: Record<string, number> = { A1: 62, A2: 60, B1: 58 }; // triệu/m2
  ...
  TOWERS.forEach((tower) => {
    for (let floor = 5; floor <= 12; floor++) {
      BEDROOMS.forEach((bedroom, bi) => {
        ...
        const floorAdj = (floor - 5) * 0.4; // giá tăng theo tầng
...
export default function UnitInventory() {
  ...
  const units = useMemo(
    () => ALL_UNITS.filter((u) => (tower === 'all' || u.tower === tower) && (status === 'all' || u.status === status)),
    [tower, status]
  );

  const summary = useMemo(() => {
    const src = ALL_UNITS.filter((u) => tower === 'all' || u.tower === tower);
    return {
      total: src.length,
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <p style={{ color: '#64748b', marginBottom: 20 }}>Trạng thái từng căn theo Tower · Tầng · Loại căn, trên nền ma trận giá dự án.</p>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 16, maxWidth: 460 }}>
      ...
      <div style={card}>
        ...
        {byFloor.length === 0 && <div style={{ textAlign: 'center', color: '#94a3b8', padding: 16 }}>Không có căn nào khớp bộ lọc.</div>}
      </div>
    </div>
  );
}
