...
const ICONS = {
    ...
    CALC: <svg className="w-5 h-5 text-sgs-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 3.659c0 3.074-1.8 5.529-4.29 6.226 1.01.83 1.87 1.96 2.42 3.26M9 15.409C10.45 14.83 11.4 13.513 11.4 12c0-1.572-1.201-2.849-2.73-2.951" /></svg>,
    CHART: <svg className="w-5 h-5 text-sgs-verified" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z" /></svg>,
    ...
    return createPortal(
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            <div className="bg-[var(--bg-surface)] w-full max-w-sm rounded-[24px] p-6 shadow-2xl border border-[var(--glass-border)] relative z-10 animate-scale-up">
                ...
                <button 
                    onClick={handleCopy} 
                    className={`w-full py-3 rounded-xl font-bold text-sm shadow-lg transition-all flex items-center justify-center gap-2 active:scale-95 ${copied ? 'bg-sgs-verified text-white' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-enter">
                        ...
                        <div className="bg-sgs-champagne rounded-2xl p-5 border border-sgs-border flex flex-col justify-center min-w-0">
                            ...
                            <div className="space-y-2 text-sm">
                                ...
                                <div className="flex justify-between items-center gap-2 flex-wrap">
                                    <span className="text-sgs-text-muted truncate">{t('calc.total_interest')}</span>
                                    <span className="font-bold text-sgs-primary break-all">{formatCurrency(totalInterest)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
...
const STATUS_CONFIG: Record<string, { color: string, bg: string, border: string }> = {
    ...
    OPENING: { color: 'text-sgs-primary', bg: 'bg-sgs-champagne', border: 'border-sgs-border' },
    AVAILABLE: { color: 'text-sgs-verified', bg: 'bg-emerald-50', border: 'border-sgs-verified/30' },
                ...
                >
                    ...
                    <button
                        ...
                        className={[
                            ...
                            !value
                                ? 'bg-sgs-champagne/30 text-sgs-primary font-semibold'
                    ...
                    >
                        ...
                        {!value && (
                            <svg className="w-3.5 h-3.5 text-sgs-primary flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        return (
                            <button
                                ...
                                className={[
                                    ...
                                    isSelected
                                        ? 'bg-sgs-champagne/30 text-sgs-primary'
                                        : 'text-[var(--text-secondary)] hover:bg-[var(--glass-surface)] hover:text-[var(--text-primary)]',
                                ].join(' ')}
                            >
                                <img src={avatarUrl(u.name, u.avatar)} alt={u.name} className="w-6 h-6 rounded-full object-cover flex-shrink-0" />
                                <div className="flex-1 min-w-0 text-left">
                                    <div className={`truncate font-medium leading-tight ${isSelected ? 'text-sgs-primary' : ''}`}>{u.name}</div>
            ...
            {isOpen && createPortal(
                ...
                >
                    <button
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-xs text-[var(--text-secondary)] hover:bg-[var(--glass-surface)] hover:text-sgs-primary transition-colors"
                    >
...
const ProjectUnits = memo(({ projectCode, parentLocation, parentContactPhone, t, formatCurrency, formatCompactNumber }: { projectCode: string, parentLocation?: string, parentContactPhone?: string, t: any, formatCurrency: any, formatCompactNumber: any }) => {
    ...
    return (
                ...
                {isAuth && (canManageUnits || userRole === 'SALES' || userRole === 'MARKETING') && (
                    <div className="flex items-center gap-2 shrink-0">
                        ...
                        <button 
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 h-[36px] bg-sgs-primary text-white text-sm font-bold rounded-xl hover:bg-sgs-primary transition-colors shadow-sm whitespace-nowrap"
                        >
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="px-4 py-3">
                                                    <span className="font-mono text-xs font-bold text-sgs-primary group-hover:text-sgs-primary">{unit.code}</span>
                                                </td>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex flex-col">
                                                <span className="font-mono text-xs2 font-bold text-sgs-primary mb-1">{unit.code}</span>
        ...
        {createPortal(
            toast ? (
                <div className={`fixed bottom-6 right-6 z-[100] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-enter border ${toast.type === 'success' ? 'bg-sgs-primary-deep border-sgs-primary text-white' : 'bg-rose-900/90 border-red-500 text-white'}`}>
...
export const ListingDetail: React.FC = () => {
    ...
    const handleBooking = async (date: string, time: string, note: string, name: string, phone: string) => {
        try {
            ...
            if (currentUser) {
                ...
                // Listing-scoped public lead — guaranteed listing attribution +
                // dedup phone+listingId 24h + 5 req/h/IP rate limit on backend.
    ...
    const handleContact = useCallback(() => {
        ...
        if (!currentUser && listing?.id) {
            // Use listing-scoped endpoint so vendor's CRM receives the lead
            // attributed to this exact listing (not generic public inbox).
    ...
    return (
        ...
        <article className="h-[100dvh] overflow-y-auto no-scrollbar bg-[var(--bg-surface)] pb-28 lg:pb-20 animate-enter relative">
            ...
            <div className="sticky top-0 z-40 bg-[var(--bg-surface)]/80 backdrop-blur-md border-b border-[var(--glass-border)] px-4 py-3 md:px-6 md:py-4 flex justify-between items-center gap-2">
                <button 
                    type="button" 
                    onClick={handleBack} 
                    className="flex items-center gap-1.5 text-sm font-bold text-[var(--text-secondary)] hover:text-sgs-primary transition-colors px-3 py-2 rounded-lg hover:bg-[var(--glass-surface)] active:bg-[var(--glass-surface-hover)] shrink-0"
                >
                ...
                <div className="flex gap-2 items-center">
                    {canManageListing && (
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 h-[36px] bg-[var(--glass-surface)] border border-[var(--glass-border)] text-[var(--text-secondary)] hover:text-sgs-primary hover:border-sgs-border hover:bg-sgs-champagne rounded-xl text-xs font-bold transition-colors"
                    ...
                    <button 
                        type="button"
                        onClick={handleShare} 
                        className="p-2 text-[var(--text-secondary)] hover:text-sgs-primary rounded-full hover:bg-[var(--glass-surface)] transition-colors" 
                        title={t('common.share_link')}
                    >
                        {ICONS.SHARE}
                    </button>
                    <button onClick={handleLogin} className="px-5 py-2 min-h-[44px] bg-sgs-primary-deep text-white font-bold rounded-xl text-xs hover:bg-slate-800 transition-colors shadow-lg active:scale-95 flex items-center justify-center">
            ...
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-2 space-y-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                        {attributes.map((attr, i) => (
                            <div key={i} className="bg-[var(--glass-surface)] p-4 rounded-2xl border border-[var(--glass-border)] hover:bg-[var(--bg-surface)] hover:shadow-md transition-all duration-300 group">
                                <div className="text-xs2 text-[var(--text-secondary)] font-bold uppercase tracking-wider mb-1 group-hover:text-sgs-primary transition-colors">{attr.label}</div>
                    ...
                    {listing.type !== PropertyType.PROJECT && (
                        <div className="flex flex-col gap-8 mt-8">
                            ...
                            <div className="bg-[var(--bg-surface)] rounded-[24px] border border-[var(--glass-border)] shadow-sm overflow-hidden flex flex-col min-w-0">
                                {/* Header */}
                                <div className="p-5 border-b border-[var(--glass-border)] flex items-center gap-3 bg-gradient-to-r from-sgs-champagne/40 to-sgs-champagne/20">
                                    <div className="w-8 h-8 bg-sgs-primary rounded-xl flex items-center justify-center text-white flex-shrink-0">
                                    ...
                                    {!isTeaserLoading && teaserData && (
                                        <div className="space-y-4 animate-enter">
                                            ...
                                            <div>
                                                ...
                                                {/* Main price range */}
                                                <div className="bg-gradient-to-r from-sgs-champagne/40 to-sgs-champagne rounded-2xl border border-sgs-border p-4">
                                                    <div className="flex items-center justify-between gap-2 flex-wrap">
                                                        <div className="text-center">
                                                            <div className="text-[10px] font-bold text-sgs-text-muted uppercase mb-0.5">Thấp</div>
                                                            <div className="text-sm font-black text-sgs-primary">{teaserData.totalMinDisplay}</div>
                                                        </div>
                                                        <div className="flex-1 h-2 mx-2 bg-sgs-champagne rounded-full relative min-w-[40px]">
                                                            <div className="absolute inset-0 bg-gradient-to-r from-sgs-primary to-sgs-primary-deep rounded-full opacity-60"></div>
                                                            <div className="absolute left-1/2 -translate-x-1/2 -top-1 w-4 h-4 bg-sgs-primary rounded-full shadow border-2 border-white"></div>
                                                        </div>
                                                        <div className="text-center">
                                                            <div className="text-[10px] font-bold text-sgs-primary uppercase mb-0.5">Cao</div>
                                                            <div className="text-sm font-black text-sgs-primary">{teaserData.totalMaxDisplay}</div>
                                                        </div>
                                                    </div>
                                                    <div className="mt-3 pt-3 border-t border-sgs-border text-center">
                                                        <span className="text-xs text-sgs-primary font-medium">Trung tâm: </span>
                                                        <span className="text-sm font-black text-sgs-primary">{teaserData.totalMidDisplay}</span>
                                                    </div>
                                                </div>
                                                {/* Meta row */}
                                                <div className="flex items-center gap-3 mt-2 flex-wrap">
                                                    <span className="inline-flex items-center gap-1 text-[10px] text-[var(--text-tertiary)] font-medium">
                                                        <svg className="w-3 h-3 text-sgs-verified" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                                            ...
                                            {/* ── Step 2a: Guest CTA ── */}
                                            {!currentUser && (
                                                <div className="rounded-2xl border border-sgs-border bg-gradient-to-br from-sgs-champagne/40 to-sgs-champagne p-5 space-y-3">
                                                    <div className="flex items-start gap-3">
                                                        <div className="w-9 h-9 bg-sgs-champagne rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5">
                                                            <Sparkles className="w-4 h-4 text-sgs-primary" />
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <p className="text-sm font-bold text-sgs-primary">Nhận báo cáo AI chi tiết — Miễn phí</p>
                                                            <p className="text-xs text-sgs-primary mt-0.5 leading-relaxed">
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full py-2.5 rounded-xl bg-sgs-primary text-white text-sm font-bold hover:bg-sgs-primary active:scale-95 transition-all flex items-center justify-center gap-2"
                                                    >
                                                        <Lock className="w-3.5 h-3.5" />
                                                        Đăng nhập để nhận báo cáo AI
                                                    </button>
                                                    <p className="text-center text-[10px] text-sgs-text-muted">Miễn phí · Không cần thẻ tín dụng</p>
                                                </div>
                                            )}
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <AiQuotaGate quota={valuationQuota} featureLabel="thẩm định AI" onUpgradeClick={() => window.open('/pricing', '_blank')}>
                                                        <button
                                                            ...
                                                            className={`w-full py-2.5 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${isValuating ? 'bg-[var(--glass-surface-hover)] text-[var(--text-secondary)]' : 'bg-sgs-primary text-white hover:bg-sgs-primary-deep active:scale-95'}`}
                                                        >
                                                            <Sparkles className="w-4 h-4" />
                                            ...
                                            {valuation && (
                                                <div className="animate-enter space-y-4">
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${valuation.isRealtime ? 'bg-sgs-champagne text-sgs-primary' : 'bg-amber-100 text-amber-700'}`}>
                                                    ...
                                                    <div className="grid grid-cols-2 gap-3">
                                                        <div className="p-3 bg-sgs-champagne rounded-xl border border-sgs-border min-w-0">
                                                            <div className="text-[10px] font-bold text-sgs-primary uppercase mb-0.5">Đơn giá /m²</div>
                                                            <div className="text-base font-black text-sgs-primary break-all">{formatCurrency(valuation.pricePerM2)}</div>
                                                        </div>
                                                        <div className={`p-3 rounded-xl border min-w-0 ${valuation.trendGrowthPct >= 0 ? 'bg-sgs-champagne/50 border-sgs-verified/30' : 'bg-rose-50 border-rose-100'}`}>
                                                            <div className={`text-[10px] font-bold uppercase mb-0.5 ${valuation.trendGrowthPct >= 0 ? 'text-sgs-verified' : 'text-rose-600'}`}>Xu hướng</div>
                                                            <div className={`text-base font-black ${valuation.trendGrowthPct >= 0 ? 'text-sgs-primary-deep' : 'text-rose-900'}`}>
                                                        ...
                                                        <div className="space-y-1.5">
                                                            ...
                                                            {valuation.factors.filter((f: any) => f.type === 'AVM' && Math.abs(f.impact) >= 2).slice(0, 4).map((f: any, i: number) => (
                                                                <div key={i} className="flex items-center justify-between gap-2 text-xs">
                                                                    ...
                                                                    <span className={`font-bold whitespace-nowrap ${f.isPositive ? 'text-sgs-verified' : 'text-rose-600'}`}>
                                                    ...
                                                    <AiQuotaGate quota={valuationQuota} featureLabel="thẩm định AI" onUpgradeClick={() => window.open('/pricing', '_blank')}>
                                                        <button
                                                            ...
                                                            className="w-full py-2 rounded-xl text-xs font-bold text-sgs-primary border border-sgs-border hover:bg-sgs-champagne transition-all flex items-center justify-center gap-1.5"
                                                        >
                    ...
                    {canViewInternalInfo && (
                        <div className="bg-sgs-primary-deep rounded-[32px] p-8 border border-slate-800 mt-12 shadow-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 w-64 h-64 bg-sgs-champagne/60 blur-[100px] -mr-32 -mt-32"></div>
                            <div className="relative z-10">
                                <div className="flex items-center gap-3 mb-8">
                                    <div className="w-10 h-10 bg-sgs-primary/20 rounded-xl flex items-center justify-center border border-sgs-primary/30">
                                        <Lock className="w-5 h-5 text-sgs-text-muted" />
                                    </div>
                                    <div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-[var(--bg-surface)]/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm">
                                        ...
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="text-sm font-mono font-bold text-sgs-text-muted truncate min-w-0">{listing.ownerPhone || '--'}</span>
                                            {listing.ownerPhone && (
                                                <button 
                                                    onClick={() => window.location.href = `tel:${listing.ownerPhone}`}
                                                    className="p-1.5 bg-sgs-primary/20 rounded-lg text-sgs-text-muted hover:bg-sgs-primary hover:text-white transition-all flex-shrink-0"
                                                >
                                                    {ICONS.PHONE}
                                                </button>
                                            )}
                                    ...
                                    <div className="bg-[var(--bg-surface)]/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm">
                                        ...
                                        <span className="text-sm font-bold text-sgs-verified">
                                    ...
                                    <div className="bg-[var(--bg-surface)]/5 border border-white/10 p-5 rounded-2xl backdrop-blur-sm">
                                        ...
                                        <span className={`text-xs2 font-bold px-2 py-1 rounded-lg uppercase tracking-wider inline-block ${listing.isVerified ? 'bg-sgs-verified/20 text-sgs-verified border border-sgs-verified/30' : 'bg-slate-700 text-[var(--text-secondary)] border border-slate-600'}`}>
                    ...
                    <div className="bg-[var(--bg-surface)] p-5 rounded-2xl border border-[var(--glass-border)] shadow-sm sticky top-24">
                        ...
                        {listing.contactPhone ? (
                            ...
                            >
                                {ICONS.PHONE}
                                {showPhone
                                    ? <><span className="w-2 h-2 rounded-full bg-sgs-verified animate-pulse flex-shrink-0"></span>{displayPhone}</>
        ...
        {createPortal(
            <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[var(--bg-surface)]/95 backdrop-blur-md border-t border-[var(--glass-border)] px-4 py-3 flex gap-3">
                <button
                    ...
                    className="flex-1 py-3.5 bg-sgs-primary-deep text-white font-bold rounded-xl text-sm flex items-center justify-center gap-2 active:scale-95 transition-all"
                >
                    {ICONS.CALENDAR} {t('detail.book_viewing')}
                </button>
                {listing.contactPhone ? (
                    <a
                        href={`tel:${listing.contactPhone}`}
                        onClick={handleContact}
                        className="flex-1 py-3.5 border rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 bg-sgs-champagne border-sgs-verified/30 text-sgs-verified hover:bg-sgs-champagne active:scale-95"
                    >
                        {ICONS.PHONE}
                        {showPhone
                            ? <><span className="w-2 h-2 rounded-full bg-sgs-verified animate-pulse flex-shrink-0"></span>{displayPhone}</>
        ...
        {createPortal(
            toast ? (
                <div className={`fixed top-6 right-4 md:right-6 z-[100] px-4 md:px-6 py-3 rounded-xl shadow-2xl flex items-center gap-3 animate-enter border max-w-[calc(100vw-2rem)] ${toast.type === 'success' ? 'bg-sgs-primary-deep border-sgs-primary text-white' : 'bg-rose-900/90 border-red-500 text-white'}`}>
