...
const ICONS = {
    ...
    DATA: <svg className="w-12 h-12 text-[#C9A84C]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" /></svg>,
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        {autoDetectedType && (
                                            <div className="flex items-center gap-1.5 mt-1">
                                                <span className="text-xs bg-[#C9A84C]/20 text-[#C9A84C] border border-[#C9A84C]/30 px-2 py-0.5 rounded-full font-bold">
                                ...
                                <div>
                                    ...
                                    <div className="grid grid-cols-3 gap-2 mb-4">
                                        {[
                                            ...
                                            <button key={opt.id} onClick={() => { setPropertyType(opt.id); setAutoDetectedType(null); }}
                                                className={`py-2 px-2 rounded-xl text-xs font-bold transition-all border text-left leading-tight ${propertyType === opt.id ? 'bg-[#C9A84C] text-white border-[#C9A84C]0' : 'bg-slate-900 text-slate-400 border-slate-700 hover:border-[#C9A84C]/50'}`}>
                                                {opt.label}
                                            </button>
                                        ))}
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-2">
                                        {[
                                            ...
                                            <button key={opt.id} onClick={() => { setPropertyType(opt.id); setAutoDetectedType(null); }}
                                                className={`py-2 px-3 rounded-xl text-xs font-bold transition-all border text-left leading-tight ${propertyType === opt.id ? 'bg-[#C9A84C] text-white border-[#C9A84C]0' : 'bg-slate-900 text-slate-400 border-slate-700 hover:border-[#C9A84C]/50'}`}>
                                                {opt.label}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                {!propertyType.startsWith('land_') && propertyType !== 'warehouse' && (
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            {(propertyType === 'office'
                                                ? [
                                                    { id: 'LUXURY', label: 'Cao cấp', badge: '+12%', badgeColor: 'text-[#C9A84C]' },
                                                ...
                                                : [
                                                    { id: 'LUXURY', label: 'Nội thất cao cấp', badge: '+12%', badgeColor: 'text-[#C9A84C]' },
                        ...
                        {teaserLoading ? (
                            ...
                            <div className="bg-slate-800 rounded-[32px] border border-slate-700 p-7 md:p-8 shadow-2xl">
                                ...
                                {!!teaser.internalCompsCount && teaser.internalCompsCount > 0 && (
                                    <div className="flex items-center gap-2 bg-[#C9A84C]/10 border border-[#C9A84C]/25 rounded-xl px-3 py-2 mb-3">
                                        <svg className="w-3.5 h-3.5 text-[#C9A84C] shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7c0-2-1-3-3-3H7C5 4 4 5 4 7zM9 12h6M9 8h6M9 16h3" /></svg>
                                        <span className="text-xs text-[#C9A84C] font-medium">
                                            Đã đối chiếu <b className="text-[#C9A84C]">{teaser.internalCompsCount} BĐS tương đương</b> từ kho nội bộ
                                            {teaser.internalCompsMedian ? (
                                                <> · median <b className="text-[#C9A84C]">{(teaser.internalCompsMedian / 1_000_000).toFixed(0)} tr/m²</b></>
                                            ) : null}
                                        </span>
                                    </div>
                                )}
                                    ...
                                    <div className="bg-slate-900/40 border border-slate-700/40 rounded-xl px-3 py-2.5 text-center">
                                        ...
                                        <div className="text-[11px] font-bold text-slate-300 leading-tight">
                                            {teaser.internalCompsCount
                                                ? <span className="text-[#C9A84C]">{teaser.internalCompsCount} BĐS</span>
                                                : teaser.dataSource
                                            }
                                        </div>
                                    </div>
                        ...
                        {valuation.marketTrend && valuation.marketTrend !== 'Đang cập nhật' && (
                            <div className="bg-slate-800 rounded-[32px] border border-slate-700 p-8 shadow-2xl mb-6 space-y-5">
                                <div>
                                    ...
                                    <p className="text-slate-300 text-sm leading-relaxed border-l-[3px] border-[#C9A84C] pl-4">
                        ...
                        {valuation.incomeApproach && (
                            <div className="bg-slate-800 rounded-[32px] border border-slate-700 p-8 shadow-2xl mb-6">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                                    ...
                                    {/* Income Method */}
                                    <div className="bg-[#FDF6E3]0/5 border border-[#C9A84C]/20 rounded-2xl p-5">
                                        <div className="flex items-center gap-2 mb-3">
                                            <span className="w-2.5 h-2.5 rounded-full bg-[#C9A84C] flex-shrink-0"></span>
                                            <span className="text-[#C9A84C] text-xs font-bold uppercase tracking-widest">Phương pháp Thu nhập & Vốn hóa</span>
                                        </div>
                                        <div className="text-2xl font-black text-white mb-1">
                                        ...
                                        {valuation.reconciliation && (
                                            <div className="mt-3 text-xs bg-[#C9A84C]/10 rounded-lg px-3 py-1.5 inline-block">
                                                Trọng số: <span className="font-bold text-[#C9A84C]">{(valuation.reconciliation.incomeWeight * 100).toFixed(0)}%</span>
                                            </div>
                                        )}
                                    </div>
                                </div>
                                    ...
                                    <>
                                        ...
                                        {feedbackRating !== null && (
                                            <button
                                                ...
                                                className="self-start px-5 py-2 bg-[#C9A84C] hover:bg-[#C9A84C] disabled:opacity-50 text-white text-sm font-semibold rounded-xl transition-colors"
                                            >
