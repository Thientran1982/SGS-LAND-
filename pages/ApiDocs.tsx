...
export const ApiDocs: React.FC = () => {
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900 h-[100dvh] flex flex-col">
            {/* Header */}
            <div className="sticky top-0 bg-white/80 backdrop-blur-md z-50 border-b border-slate-200 shrink-0">
            ...
            <div className="flex-1 overflow-hidden flex max-w-[1440px] mx-auto w-full">
                {/* Sidebar */}
                <div className="w-64 border-r border-slate-200 bg-white hidden md:block overflow-y-auto py-8 px-6 no-scrollbar">
                        ...
                        {['intro', 'auth', 'endpoints', 'errors'].map(sec => (
                            <li key={sec}>
                                <button 
                                    ...
                                    className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${activeSection === sec ? 'bg-indigo-50 text-indigo-700' : 'text-slate-600 hover:bg-slate-50'}`}
                                >
                                    {t(`api.${sec}`)}
                                </button>
                            </li>
                ...
                <div className="flex-1 overflow-y-auto p-8 md:p-12 scroll-smooth no-scrollbar">
                    <div className="max-w-3xl">
                        <div className="mb-12">
                            <h1 className="text-4xl font-black text-slate-900 mb-4">{t('api.title')}</h1>
                            <p className="text-lg text-slate-500 leading-relaxed">{t('api.subtitle')}</p>
                        </div>

                        <section id="auth" className="mb-16">
                            <h2 className="text-2xl font-bold text-slate-900 mb-4">{t('api.auth')}</h2>
                            <p className="text-slate-600 mb-4">SGS Land API sử dụng API key để xác thực yêu cầu. Bạn có thể xem và quản lý key trong phần Cài đặt Doanh nghiệp.</p>
                            <CodeBlock code={`Authorization: Bearer sgs_live_...`} language="http" />
                        </section>

                        <section id="endpoints" className="mb-16">
                            <h2 className="text-2xl font-bold text-slate-900 mb-4">Endpoint Khách Hàng</h2>
                            <div className="flex items-center gap-3 mb-4">
                                <span className="px-3 py-1 bg-emerald-100 text-emerald-700 font-bold rounded-lg text-xs font-mono">GET</span>
                                <code className="text-sm font-mono text-slate-700">/v1/leads</code>
                            </div>
                            <p className="text-slate-600 mb-4">Trả về danh sách khách hàng tiềm năng liên kết với tài khoản của bạn.</p>
                            
                            <CodeBlock code={`curl https://api.sgs.vn/v1/leads \\
  -H "Authorization: Bearer sgs_live_..." \\
  -H "Content-Type: application/json"`} />

                            <h3 className="font-bold text-slate-800 mt-6 mb-2">Phản hồi mẫu</h3>
                            <CodeBlock code={`{
  "data": [
    {
      "id": "lead_123",
