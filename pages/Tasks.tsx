...
function TaskList() {
  ...
  const SortIcon = ({ col }: { col: SortKey }) => {
    ...
    return sortDir === 'asc' ? <ArrowUp size={12} className="text-[#C9A84C]0" /> : <ArrowDown size={12} className="text-[#C9A84C]0" />;
  };
    ...
    <div className="h-full flex flex-col overflow-hidden animate-enter">
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3.5 border-b border-[var(--glass-border)] flex-shrink-0 gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <ListTodo className="w-5 h-5 text-[#C9A84C] flex-shrink-0" />
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => exportTasksToCSV(tasks)} title="Xuất CSV"
            className="h-[34px] w-[34px] flex items-center justify-center border border-[var(--glass-border)] rounded-xl text-[var(--text-tertiary)] hover:text-[#C9A84C] hover:border-[#C9A84C] transition-colors">
            <Download size={15} />
          </button>
          <button onClick={() => setShowCreate(true)}
            className="h-[34px] px-3.5 bg-[#C9A84C] text-white rounded-xl text-sm font-semibold flex items-center gap-1.5 hover:bg-[#B8860B] transition-colors shadow-sm flex-shrink-0">
            <Plus size={15} /> Thêm
          </button>
        </div>
      </div>
      ...
      {/* Bulk action bar */}
      {selectedIds.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 px-4 md:px-6 py-2 bg-[#FDF6E3] dark:bg-[#1C2B4A]/20 border-b border-[#C9A84C] dark:border-[#1C2B4A] flex-shrink-0">
          <span className="text-xs font-semibold text-[#B8860B] dark:text-[#C9A84C]">{selectedIds.size} đã chọn</span>
          <div className="flex flex-wrap gap-2 ml-auto items-center">
            {/* Bulk assign picker */}
            <div className="relative" ref={bulkAssignRef}>
              <button
                onClick={() => { setBulkAction('assign'); setBulkAssignPickerOpen(v => !v); }}
                className={`h-[28px] px-2.5 text-xs border rounded-lg flex items-center gap-1.5 transition-colors font-medium ${bulkAction === 'assign' ? 'border-[#C9A84C] bg-[#FDF6E3] dark:bg-[#1C2B4A]/30 text-[#B8860B]' : 'border-[#C9A84C] dark:border-[#B8860B] text-[#C9A84C] hover:bg-[#FDF6E3]'}`}>
            ...
            <button onClick={runBulkAction} disabled={!bulkAction || bulkAction === 'assign' || bulkLoading}
              className="h-[28px] px-3 text-xs bg-[#C9A84C] text-white rounded-lg font-medium flex items-center gap-1 hover:bg-[#B8860B] disabled:opacity-50 transition-colors">
      ...
      <div className="flex-1 overflow-auto no-scrollbar">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-32 gap-2">
            ...
            <button onClick={() => loadTasks(filters, sortKey, sortDir, page)} className="text-xs text-[#C9A84C] hover:text-[#C9A84C] font-medium">Thử lại</button>
          </div>
        ) : tasks.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-40 gap-3">
            ...
            {!hasFilters && (
              <button onClick={() => setShowCreate(true)} className="text-sm text-[#C9A84C] hover:text-[#C9A84C] font-medium flex items-center gap-1">
            ...
            <thead className="sticky top-0 bg-[var(--bg-surface)] border-b border-[var(--glass-border)] z-10">
              <tr className="text-left text-[var(--text-tertiary)]">
                <th className="pl-4 md:pl-6 pr-2 py-3 w-8">
                  <button onClick={toggleSelectAll} className="text-[var(--text-tertiary)] hover:text-[#C9A84C] transition-colors">
                    {allSelected ? <CheckSquare size={15} className="text-[#C9A84C]0" /> : <Square size={15} />}
                  </button>
                </th>
              ...
              {tasks.map(task => (
                <tr key={task.id} className="hover:bg-[var(--glass-surface-hover)] transition-colors group">
                  <td className="pl-4 md:pl-6 pr-2 py-3 w-8" onClick={e => { e.stopPropagation(); toggleSelect(task.id); }}>
                    <button className="text-[var(--text-tertiary)] hover:text-[#C9A84C] transition-colors">
                      {selectedIds.has(task.id) ? <CheckSquare size={15} className="text-[#C9A84C]0" /> : <Square size={15} />}
                    </button>
                  </td>
                  <td className="px-2 py-3 cursor-pointer" onClick={() => openTask(task.id)}>
                    <div className="font-medium text-[var(--text-primary)] line-clamp-1 group-hover:text-[#C9A84C] transition-colors">{task.title}</div>
                  ...
                  <td className="px-3 py-3 hidden sm:table-cell cursor-pointer" onClick={() => openTask(task.id)}>
                    <ChevronRight size={14} className="text-[var(--text-tertiary)] group-hover:text-[#C9A84C] transition-colors" />
                  </td>
                </tr>
              ))}
            </tbody>
