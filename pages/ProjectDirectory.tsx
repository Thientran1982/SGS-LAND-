...
const ProjectCard = ({ project }: { project: DuAnProject }) => (
    ...
    >
        ...
        <div className="p-5 flex flex-col flex-1">
            <span className="text-xs font-bold text-[#C9A84C] uppercase tracking-wide mb-2">{project.projectType}</span>
            <h3 className="text-base font-bold text-[var(--text-primary)] mb-1 group-hover:text-[#C9A84C] transition-colors leading-snug">
                {project.name}
            </h3>
            ...
            <div className="mt-auto pt-3 border-t border-[var(--glass-border)] flex items-center justify-between">
                <span className="text-sm font-extrabold text-[#C9A84C]">{project.priceRange}</span>
...
export default function ProjectDirectory() {
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-app)] text-[var(--text-primary)]">
            ...
            <nav className="sticky top-0 z-40 border-b border-[var(--glass-border)] bg-[var(--bg-surface)]/90 backdrop-blur-md">
                <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
                    ...
                    <nav className="hidden md:flex items-center gap-5 text-xs font-semibold text-[var(--text-secondary)]">
                        <button onClick={() => navigate('/marketplace')} className="hover:text-[#C9A84C] transition-colors">Mua Bán BĐS</button>
                        <button onClick={() => navigate('/ai-valuation')} className="hover:text-[#C9A84C] transition-colors">Định Giá AI</button>
                        <button onClick={() => navigate('/ky-gui-bat-dong-san')} className="hover:text-[#C9A84C] transition-colors">Ký Gửi</button>
                    </nav>
                    <a
                        href={`tel:${HOTLINE_TEL}`}
                        className="flex items-center gap-2 px-4 py-2 bg-[#C9A84C] text-white rounded-xl text-xs font-bold hover:bg-[#B8860B] transition-colors shrink-0"
                    >
            ...
            {/* ── Hero ── */}
            <section className="relative overflow-hidden bg-gradient-to-br from-[#0D1829] via-[#1C2B4A] to-[#1A2332] text-white py-16 px-4">
                <div className="absolute inset-0 pointer-events-none">
                    <div className="absolute -top-20 -left-20 w-96 h-96 rounded-full bg-[#C9A84C]/10 blur-3xl" />
                    <div className="absolute bottom-0 right-0 w-64 h-64 rounded-full bg-[#C9A84C]/10 blur-2xl" />
                </div>
                <div className="relative max-w-4xl mx-auto text-center">
                    <p className="text-xs font-bold text-[#E8D4A0] uppercase tracking-widest mb-4">
                        SGS LAND · Đại Lý Phân Phối Chính Thức
                    </p>
                    <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight mb-4 leading-tight">
                        Dự Án BĐS<br />
                        <span className="text-[#C9A84C]">SGS LAND Đang Phân Phối</span>
                    </h1>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        ...
                        <div className="relative flex-1 max-w-sm">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 rounded-xl text-sm border border-[var(--glass-border)] bg-[var(--bg-app)] text-[var(--text-primary)] placeholder-[var(--text-tertiary)] focus:outline-none focus:ring-2 focus:ring-[#C9A84C] transition"
                            />
                        </div>
                        {/* Filter toggle (mobile) */}
                        <button
                            onClick={() => setShowFilters(v => !v)}
                            className={`sm:hidden flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border transition-colors ${hasActiveFilters ? 'border-[#C9A84C] text-[#C9A84C] bg-[#FDF6E3]' : 'border-[var(--glass-border)] text-[var(--text-secondary)] bg-[var(--bg-app)]'}`}
                        >
                ...
                {filtered.length === 0 ? (
                    <div className="text-center py-20">
                        ...
                        <button
                            ...
                            className="px-6 py-2.5 bg-[#C9A84C] text-white rounded-xl text-sm font-bold hover:bg-[#B8860B] transition-colors"
                        >
                            Xem tất cả dự án
                        </button>
                    </div>
            ...
            {/* ── CTA ── */}
            <section className="bg-gradient-to-br from-[#1C2B4A] to-[#0F1C33] text-white py-14 px-4 mt-4">
                <div className="max-w-3xl mx-auto text-center">
                    <p className="text-xs font-bold uppercase tracking-widest text-white/60 mb-3">Tư Vấn Miễn Phí · Không Thu Phí Người Mua</p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 justify-center">
                        <a
                            href={`tel:${HOTLINE_TEL}`}
                            className="flex items-center justify-center gap-2 px-8 py-4 bg-white text-[#B8860B] rounded-2xl font-extrabold text-base hover:bg-[#FDF6E3] transition-all shadow-lg"
                        >
            ...
            <footer className="border-t border-[var(--glass-border)] bg-[var(--bg-surface)] py-8 px-4">
                <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-[var(--text-secondary)]">
                        <button onClick={() => navigate('/marketplace')} className="hover:text-[#C9A84C]">Mua Bán BĐS</button>
                        <button onClick={() => navigate('/ai-valuation')} className="hover:text-[#C9A84C]">Định Giá AI</button>
                        <button onClick={() => navigate('/ky-gui-bat-dong-san')} className="hover:text-[#C9A84C]">Ký Gửi BĐS</button>
                        <button onClick={() => navigate('/news')} className="hover:text-[#C9A84C]">Tin Tức</button>
                        <button onClick={() => navigate('/contact')} className="hover:text-[#C9A84C]">Liên Hệ</button>
                    </div>
                </div>
            </footer>
        </div>
        ...
        <div ref={ref} className={`relative ${fullWidth ? 'w-full' : ''}`}>
            ...
            <button
                ...
                className={`${fullWidth ? 'w-full' : ''} flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold border transition-all whitespace-nowrap select-none
                    ${isActive
                        ? 'border-[#C9A84C] text-[#C9A84C] bg-[#FDF6E3] dark:bg-[#1C2B4A]/60 dark:text-[#E8D4A0] dark:border-[#C9A84C]/50'
                        : 'border-[var(--glass-border)] text-[var(--text-secondary)] bg-[var(--bg-app)] hover:border-[#C9A84C]/60 hover:text-[#C9A84C]'
                    } ${open ? 'ring-2 ring-[#C9A84C]/30' : ''}`}
            >
                <span className={`${isActive ? 'text-[#C9A84C]' : 'text-[var(--text-tertiary)]'}`}>
                    {icon}
                </span>
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-3 py-2.5 text-xs text-left transition-colors
                                        ${isSelected
                                            ? 'bg-[#FDF6E3] text-[#B8860B] font-bold dark:bg-[#1C2B4A]/60 dark:text-[#E8D4A0]'
