    ...
    return (
      <div className="min-h-[100dvh] bg-slate-50 flex items-center justify-center">
        <div className="flex flex-col items-center gap-3 text-slate-500">
          <div className="w-10 h-10 border-3 border-slate-300 border-t-[#C9A84C] rounded-full animate-spin" />
    ...
    return (
      <div className="min-h-[100dvh] bg-slate-50 flex items-center justify-center p-6">
        <div className="max-w-md text-center bg-white rounded-2xl shadow-lg p-8 border border-slate-200">
          ...
          <a href="/"
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#C9A84C] text-white font-semibold text-sm hover:bg-[#B8860B] transition">
            ← Về trang chủ
          </a>
        </div>
      </div>
    ...
    >
      ...
      <section className="relative bg-slate-900 text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
          <div className="flex items-center gap-3 text-xs text-white/60 font-mono uppercase tracking-widest mb-3">
            {brandLogo ? (
              <img
                src={brandLogo}
                alt={brandLabel}
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
              {gallery.map((img, i) => (
                <button key={i} type="button"
                  ...
                  className={`shrink-0 rounded-xl overflow-hidden border-2 transition ${i === activeImageIdx ? 'border-[#C9A84C] ring-2 ring-[#C9A84C]/40' : 'border-transparent hover:border-slate-300'}`}>
                  ...
                  <ul className="space-y-2">
                    {project.metadata.amenities.slice(0, 12).map((a, i) => (
                      <li key={i} className="flex items-center gap-2 text-sm text-slate-700">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#FDF6E3]0 shrink-0" />{a}
                      </li>
                    ))}
                  </ul>
                </>
      ...
      <section id="lead-form" className="max-w-6xl mx-auto px-4 sm:px-6 mt-10 mb-12">
        <div className="bg-gradient-to-br from-[#1C2B4A] to-[#0F1C33] rounded-2xl p-6 sm:p-8 text-white shadow-xl">
          <div className="grid lg:grid-cols-5 gap-6">
            ...
            <form onSubmit={handleSubmit} className="lg:col-span-3 grid sm:grid-cols-2 gap-3">
              <input
                ...
                className="px-4 py-3 rounded-xl bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/50"
                autoComplete="name" />
              <input
              ...
              <input
                ...
                className="sm:col-span-2 px-4 py-3 rounded-xl bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/50"
                autoComplete="email" />
              <select value={form.interest}
                onChange={(e) => setForm(f => ({ ...f, interest: e.target.value }))}
                className="sm:col-span-2 px-4 py-3 rounded-xl bg-white text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/50">
              ...
              <textarea value={form.note} rows={3}
                ...
                className="sm:col-span-2 px-4 py-3 rounded-xl bg-white text-slate-900 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-[#C9A84C]/50 resize-none" />
              {data?.captcha && (
              ...
              <button type="submit" disabled={submitting}
                className="sm:col-span-2 px-6 py-3 rounded-xl bg-white text-[#B8860B] font-bold text-sm hover:bg-[#FDF6E3] disabled:opacity-60 transition shadow-lg">
                {submitting ? 'Đang gửi…' : 'Gửi yêu cầu tư vấn'}
              </button>
              <p className="sm:col-span-2 text-xs text-white/60 text-center">
...
const ListingsTable: React.FC<{ listings: PublicListing[] }> = ({ listings }) => (
  <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      ...
      <table className="min-w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {listings.map((l) => {
            ...
            return (
              <tr key={l.id} className="hover:bg-[#FDF6E3]/20 transition">
                ...
                <td className="px-4 py-3 text-right font-bold text-[#B8860B]">{formatPrice(l.price, l.currency)}</td>
    ...
    <div className="px-4 py-3 bg-slate-50 border-t border-slate-200 text-center">
      <a href="#lead-form" className="text-sm text-[#B8860B] font-semibold hover:underline">
