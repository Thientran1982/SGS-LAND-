        ...
        <div className="fixed inset-0 z-[100] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 animate-enter">
            <div className="bg-white w-full max-w-md rounded-[24px] shadow-2xl p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-slate-800">{t('security.modal_add_title')}</h3>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 rounded-full text-slate-400">{ICONS.CLOSE}</button>
                </div>
                <div className="space-y-4">
                    <div>
                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">{t('security.label_rule_name')}</label>
                    ...
                    <div>
                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">{t('security.label_pattern')}</label>
                    ...
                    <div>
                        <label className="text-xs font-bold text-slate-500 uppercase block mb-1">{t('security.label_action')}</label>
                        <select className="w-full border rounded-xl px-4 py-2 text-sm bg-white focus:ring-2 focus:ring-indigo-500/20 outline-none" value={form.action} onChange={e => setForm({...form, action: e.target.value})}>
    ...
    return (
        <div className="space-y-6 pb-20 animate-enter relative">
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
                <div>
                    <h2 className="text-xl font-bold text-slate-800">{t('security.title')}</h2>
                    <p className="text-sm text-slate-500">{t('security.subtitle')}</p>
                </div>
                <div className="flex bg-slate-100 p-1 rounded-xl">
                    <button onClick={() => setActiveTab('POLICIES')} className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'POLICIES' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}>{t('security.tab_policies')}</button>
                    <button onClick={() => setActiveTab('ACCESS')} className={`px-6 py-2 rounded-lg text-xs font-bold transition-all ${activeTab === 'ACCESS' ? 'bg-white shadow text-indigo-600' : 'text-slate-500 hover:text-slate-700'}`}>{t('security.tab_access')}</button>
                </div>
            </div>
            ...
            {activeTab === 'POLICIES' && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-enter">
                    ...
                    <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="font-bold text-slate-800">{t('security.retention')}</h3>
                        ...
                        <div className="space-y-4">
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('security.msg_days')}</label>
                                <input 
                                    type="number" 
                                    ...
                                    className="w-full border rounded-xl px-4 py-2 mt-1 text-sm outline-none focus:border-indigo-500 disabled:opacity-50 disabled:bg-slate-100" 
                                />
                            </div>
                            <div>
                                <label className="text-xs font-bold text-slate-500 uppercase">{t('security.log_days')}</label>
                                <input 
                                    type="number" 
                                    disabled={config.legalHold} 
                    ...
                    <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="font-bold text-slate-800">{t('security.dlp_rules')}</h3>
                            <button onClick={() => setIsEditorOpen(true)} className="p-2 hover:bg-slate-100 rounded-full text-slate-400">{ICONS.ADD}</button>
                        </div>
                            ...
                            {(config.dlpRules || []).map(rule => (
                                <div key={rule.id} className="p-4 border rounded-xl flex justify-between items-center group hover:border-indigo-200 transition-colors">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded border ${rule.action === 'BLOCK' ? 'bg-rose-50 border-rose-100 text-rose-600' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
            ...
            {activeTab === 'ACCESS' && (
                <div className="bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm animate-enter">
                    <h3 className="font-bold text-slate-800 mb-6">{t('security.active_sessions')}</h3>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                ...
                                {sessions?.map(s => (
                                    <tr key={s.id} className="hover:bg-slate-50 transition-colors">
                                        <td className="p-3 font-bold text-slate-700">{s.userId}</td>
                                        <td className="p-3 font-mono text-slate-600">{s.ipAddress}</td>
                                        <td className="p-3 text-slate-600">{s.deviceType}</td>
                                        <td className="p-3 text-slate-500">{formatDateTime(s.lastActiveAt)}</td>
