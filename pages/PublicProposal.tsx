...
const ICONS = {
    ...
    SECURITY: <svg className="w-4 h-4 text-sgs-verified" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" /></svg>,
...
export const PublicProposal: React.FC<PublicProposalProps> = ({ token }) => {
    ...
    if (!proposal) return (
        <div className="h-screen flex flex-col items-center justify-center text-sgs-text-muted gap-4">
                ...
                <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            <Logo className="w-5 h-5 text-sgs-primary" />
                            <span className="font-bold text-slate-800 text-sm">SGS LAND</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-1.5 text-xs font-bold text-sgs-verified bg-sgs-champagne px-2.5 py-1.5 rounded-full border border-emerald-100">
                        {ICONS.SECURITY}
            ...
            <div className="max-w-2xl mx-auto px-4 py-6 pb-16 space-y-4">
                ...
                <div className="bg-white rounded-3xl shadow-sm shadow-slate-200 border border-slate-100 overflow-hidden">
                    ...
                    <div className="relative w-full aspect-[16/9] bg-slate-100 overflow-hidden">
                        ...
                        {listingType && (
                            <div className="absolute top-4 left-4">
                                <span className="text-[11px] font-bold bg-sgs-primary text-white px-2 py-1 rounded-lg uppercase tracking-wide">
                    ...
                    <div className="px-5 py-4 flex flex-wrap gap-2 border-b border-slate-100">
                        {listingArea > 0 && (
                            <span className="inline-flex items-center gap-1.5 text-sm text-sgs-text-muted bg-sgs-bg border border-slate-200 px-3 py-1.5 rounded-xl font-semibold">
                    ...
                    <div className="px-5 py-5 space-y-5">
                        ...
                        <div>
                            ...
                            <p className="text-base font-bold text-sgs-primary">
                                {leadName || '—'}
                            </p>
                        </div>
                        {/* Agent Note */}
                        {note && (
                            <div className="p-4 bg-sgs-champagne border border-sgs-border rounded-2xl">
                                <div className="text-[10px] font-bold text-sgs-text-muted uppercase tracking-wider mb-1.5">
                        ...
                        <div className="border-t border-slate-100 pt-5">
                            ...
                            <div className="text-4xl md:text-5xl font-black text-sgs-text tracking-tight">
                            ...
                            <div className="mt-3 flex flex-wrap gap-x-4 gap-y-2">
                                {!isExpired && (
                                    <div className="flex items-center gap-1.5 text-sm text-sgs-accent-text font-semibold">
                                        {ICONS.CLOCK}
                                        {t('pub.valid_until')}: <span className="font-bold">{formatDate(proposal.validUntil)}</span>
                                    </div>
                                )}
                                {depositRequired > 0 && (
                                    <div className="flex items-center gap-1.5 text-sm text-sgs-text-muted font-semibold">
                                        {ICONS.COIN}
                        ...
                        {paymentSchedule.length > 0 && (
                            <div className="border-t border-slate-100 pt-5">
                                <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                    <span className="w-5 h-5 bg-sgs-champagne rounded-full inline-flex items-center justify-center">
                                        <svg className="w-3 h-3 text-sgs-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                                    </span>
                                    Tiến độ thanh toán đề xuất
                                </p>
                                <div className="space-y-3">
                                    {paymentSchedule.map((m: any, i: number) => (
                                        <div key={m.id || i} className="flex items-center gap-3 bg-sgs-bg border border-slate-200 rounded-xl px-4 py-3">
                                            <div className="w-7 h-7 rounded-full bg-sgs-champagne text-sgs-primary flex items-center justify-center text-xs font-black shrink-0">
                                                {i + 1}
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <div className="flex justify-between items-baseline gap-2">
                                                    <span className="text-sm font-bold text-slate-800 truncate">{m.label}</span>
                                                    <span className="text-sm font-extrabold text-sgs-text shrink-0">{formatCurrency(m.amount)}</span>
                                                </div>
                                                <div className="flex justify-between text-xs text-slate-400 mt-0.5">
                                                    ...
                                                    <span className="font-mono font-bold text-sgs-primary">{m.percentage}%</span>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
                    ...
                    <div className="p-5 space-y-4">
                        ...
                        {accepted ? (
                            <div className="text-center py-4">
                                <div className="w-14 h-14 bg-sgs-champagne rounded-full flex items-center justify-center mx-auto mb-3">
                                    <svg className="w-7 h-7 text-sgs-verified" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                                </div>
                                <h3 className="text-lg font-bold text-emerald-800 mb-1">{t('pub.thank_you')}</h3>
                                <p className="text-sm text-sgs-verified">{t('pub.agent_contact_soon')}</p>
                            </div>
                        ) : (
                            <>
                                <div>
                                    <h3 className="font-bold text-slate-800 mb-1">Quan tâm đến sản phẩm này?</h3>
                                    <p className="text-sm text-sgs-text-muted">Nhấn nút bên dưới để chuyên viên liên hệ hỗ trợ bạn.</p>
                                </div>
                                <button
                                    onClick={async () => {
