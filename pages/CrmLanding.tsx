...
const ICONS = {
    ...
    CHECK: <Check className="w-5 h-5 text-sgs-primary" strokeWidth={3} />,
    CHECK_LIGHT: <Check className="w-5 h-5 text-emerald-400" strokeWidth={3} />,
    ROCKET: <Rocket className="w-6 h-6 text-white" />,
    AI_BRAIN: <BrainCircuit className="w-6 h-6 text-sgs-primary" />,
    AUTOMATION: <Zap className="w-6 h-6 text-sgs-verified" />,
    ...
    return (
        <div className="min-h-screen bg-[var(--bg-surface)] font-sans text-[var(--text-primary)] pb-20 overflow-y-auto h-[100dvh] no-scrollbar">
            ...
            <div className="sticky top-0 bg-[var(--bg-surface)]/90 backdrop-blur-md z-50 border-b border-[var(--glass-border)]">
                <div className="max-w-[1440px] mx-auto px-4 md:px-6 h-16 md:h-20 flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                        <Logo className="w-6 h-6 sm:w-8 sm:h-8 text-sgs-primary shrink-0" />
                        <span className="font-bold text-lg sm:text-xl tracking-tight hidden sm:inline truncate">SGS CRM</span>
                    </div>
                    <button onClick={handleLogin} className="px-3 sm:px-6 py-2 sm:py-2.5 bg-sgs-primary-deep text-white font-bold rounded-xl hover:bg-slate-800 transition-colors shadow-lg active:scale-95 text-xs sm:text-sm min-h-[44px] shrink-0 whitespace-nowrap">
            ...
            <section className="pt-24 pb-32 px-6 text-center max-w-5xl mx-auto animate-enter">
                <span className="inline-block py-1.5 px-4 rounded-full bg-sgs-champagne border border-sgs-border text-sgs-primary text-xs font-bold uppercase tracking-widest mb-8">
                    {t('crm.hero_badge')}
                </span>
                ...
                <div className="rounded-[32px] overflow-hidden shadow-2xl border border-[var(--glass-border)] bg-[var(--glass-surface)] relative group">
                    <div className="absolute top-0 left-0 right-0 h-12 bg-[var(--bg-surface)] border-b border-[var(--glass-border)] flex items-center px-4 gap-2">
                        <div className="flex gap-1.5">
                            ...
                            <div className="w-3 h-3 rounded-full bg-sgs-accent"></div>
                ...
                <div className="max-w-6xl mx-auto px-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
                        <div className="space-y-4">
                            <div className="w-14 h-14 bg-sgs-champagne rounded-2xl flex items-center justify-center border border-sgs-border shadow-sm">
                                {ICONS.AI_BRAIN}
                            </div>
                        ...
                        <div className="space-y-4">
                            <div className="w-14 h-14 bg-sgs-champagne rounded-2xl flex items-center justify-center border border-emerald-100 shadow-sm">
                                {ICONS.AUTOMATION}
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                    <div className="bg-[var(--bg-surface)] p-8 rounded-[32px] border border-[var(--glass-border)] shadow-sm hover:shadow-xl transition-all">
                        ...
                        <ul className="space-y-3 mb-8">
                            <li className="flex gap-3 text-[var(--text-secondary)]"><span className="text-sgs-verified">{ICONS.CHECK}</span> {t('crm.plan_basic_f1')}</li>
                            <li className="flex gap-3 text-[var(--text-secondary)]"><span className="text-sgs-verified">{ICONS.CHECK}</span> {t('crm.plan_basic_f2')}</li>
                            <li className="flex gap-3 text-[var(--text-secondary)]"><span className="text-sgs-verified">{ICONS.CHECK}</span> {t('crm.plan_basic_f3')}</li>
                        </ul>
                    ...
                    <div className="bg-sgs-primary-deep text-white p-8 rounded-[32px] shadow-2xl relative overflow-hidden transform hover:-translate-y-2 transition-all">
                        <div className="absolute top-0 right-0 bg-sgs-primary text-white text-xs2 font-bold px-3 py-1 rounded-bl-xl uppercase tracking-wider">{t('crm.plan_pro_badge')}</div>
                        <h3 className="text-lg font-bold text-sgs-on-dark-muted uppercase tracking-wide mb-2">{t('crm.plan_pro')}</h3>
                        ...
                        <button onClick={handleLogin} className="w-full py-3 bg-sgs-primary hover:bg-sgs-primary text-white font-bold rounded-xl transition-colors shadow-lg">
