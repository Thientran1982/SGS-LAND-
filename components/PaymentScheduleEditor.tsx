...
const STATUS_CONFIG: Record<PaymentStatus, { label_key: string; color: string; bg: string; dot: string }> = {
    ...
    [PaymentStatus.PAID]:     { label_key: 'payment.status_PAID',     color: 'text-[#B8860B]', bg: 'bg-[#FDF6E3] border-[#C9A84C]', dot: 'bg-[#FDF6E3]0' },
...
export const PaymentScheduleEditor: React.FC<PaymentScheduleEditorProps> = ({
    ...
    const inputCls = "w-full border border-[var(--glass-border)] rounded-lg px-2.5 py-1.5 text-xs focus:ring-2 focus:ring-[#C9A84C]/20 focus:border-[#C9A84C] outline-none transition-all bg-[var(--glass-surface)] focus:bg-[var(--bg-surface)] disabled:opacity-60 disabled:cursor-not-allowed";
    return (
        <div className="space-y-4">
            ...
            {milestones.length > 0 && (
                <div className="bg-[var(--glass-surface)] border border-[var(--glass-border)] rounded-2xl p-4 space-y-3">
                    <div className="flex items-center justify-between text-xs font-bold text-[var(--text-secondary)] uppercase tracking-wider">
                        ...
                        <span className={overdueCount > 0 ? 'text-rose-600' : 'text-[#C9A84C]'}>
                            {progressPct}%
                        </span>
                    </div>
                    {/* Progress bar */}
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden" role="progressbar" aria-valuenow={progressPct} aria-valuemin={0} aria-valuemax={100} aria-label={t('payment.progress')}>
                        <div
                            className={`h-full rounded-full transition-all duration-700 ${overdueCount > 0 ? 'bg-rose-500' : 'bg-[#FDF6E3]0'}`}
                            style={{ width: `${progressPct}%` }}
                        />
                    </div>
                    <div className="flex items-center justify-between text-xs text-[var(--text-tertiary)]">
                        <span>{t('payment.total_paid')}: <strong className="text-[#C9A84C]">{formatVND(totalPaid)}</strong></span>
                        <span>{t('payment.total_scheduled')}: <strong>{formatVND(totalScheduled)}</strong></span>
                    </div>
                    {milestones.length > 0 && (
                        <div className={`flex items-center justify-between text-xs px-3 py-2 rounded-lg border ${isOverAllocated ? 'bg-rose-50 border-rose-200 text-rose-700' : remainingPct < 0.1 ? 'bg-[#FDF6E3] border-[#C9A84C] text-[#B8860B]' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                            <span>
                        ...
                        return (
                            <div key={ms.id} className={`border rounded-xl p-4 space-y-3 transition-all ${isOverdue ? 'border-rose-200 bg-rose-50/30' : 'border-[var(--glass-border)] bg-[var(--bg-surface)]'}`}>
                                ...
                                <div className="flex items-center gap-3">
                                    <span className="w-6 h-6 rounded-full bg-[#FDF6E3] text-[#B8860B] font-bold text-xs flex items-center justify-center shrink-0">
                                        {idx + 1}
                                    </span>
                                    <input
                                        value={ms.name}
                                ...
                                {(ms.status === PaymentStatus.PAID || ms.status === PaymentStatus.WAIVED) && (
                                    <div className="grid grid-cols-2 gap-3 pt-1 border-t border-[var(--glass-border)]">
                                        <div>
                                            <label className="block text-xs font-bold text-[#C9A84C] uppercase tracking-wider mb-1">{t('payment.paid_date')}</label>
                                            <input
                                                type="date"
                                                value={ms.paidDate || ''}
                                        ...
                                        <div>
                                            <label className="block text-xs font-bold text-[#C9A84C] uppercase tracking-wider mb-1">{t('payment.paid_amount')} (VNĐ)</label>
                                            <div className="relative">
                                                <input
                                                    type="number"
                                                ...
                                                />
                                                <span className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-bold text-[#C9A84C] pointer-events-none">đ</span>
                                            </div>
                                            {(ms.paidAmount ?? ms.amount) ? (
                                                <p className="mt-1 text-[10px] font-semibold text-[#C9A84C]">
                                ...
                                {ms.amount > 0 && (
                                    <div className="text-right text-xs font-bold text-[#C9A84C]">
                                        ≈ {formatVND(ms.amount)}
            ...
            {!readOnly && (
                <button
                    ...
                    className="w-full py-2.5 rounded-xl border-2 border-dashed border-[#C9A84C] text-[#C9A84C] hover:border-[#C9A84C] hover:bg-[#FDF6E3]/50 hover:text-[#B8860B] text-xs font-bold transition-all flex items-center justify-center gap-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A84C]0"
                >
