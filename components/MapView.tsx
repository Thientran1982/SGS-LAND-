...
// Priority: PropertyType.PROJECT → dark-gold | TransactionType.RENT → navy | default → gold
...
function pinTokens(transaction?: string, propertyType?: string) {
    if (propertyType?.toUpperCase() === 'PROJECT')
        return { bg: '#B8860B', glow: 'rgba(184,134,11,0.40)' };       // blue-600 — Dự án
    if (transaction?.toUpperCase() === 'RENT')
        return { bg: '#1C2B4A', glow: 'rgba(28,43,74,0.40)' };      // violet-600 — Thuê
    return { bg: '#C9A84C', glow: 'rgba(201,168,76,0.40)' };           // indigo-600 — Bán
}
// ── Geo utilities ────────────────────────────────────────────────────────────
