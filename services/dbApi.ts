...
class DatabaseApiClient {
  ...
  async getListingStats() {
    try {
      ...
      return { availableCount: 0, holdCount: 0, soldCount: 0, rentedCount: 0, bookingCount: 0, openingCount: 0, inactiveCount: 0, otherCount: 0, totalCount: 0 };
    }
  }
  ...
  // ===== Custom Fields (Truong Tuy Chinh) =====
  async getCustomFields(entity?: string) {
    const qs = entity ? `?entity=${encodeURIComponent(entity)}` : '';
    const res = await fetch(`/api/custom-fields${qs}`, { credentials: 'include' });
    if (!res.ok) throw new Error('Failed to fetch custom fields');
    return res.json();
  }
  async createCustomField(data: { label: string; entity: string; fieldType: string; required: boolean }) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error((err as any).error || 'Failed to create custom field');
    }
    return res.json();
  }
  ...
  async getUnits(filters: { tower?: string; status?: string } = {}) {
    const params = new URLSearchParams();
    if (filters.tower) params.set('tower', filters.tower);
    if (filters.status) params.set('status', filters.status);
    const qs = params.toString() ? `?${params.toString()}` : '';
    const res = await fetch(`/api/units${qs}`, { credentials: 'include' });
  ...
  async createUnit(data: any) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error((err as any).error || 'Failed to create unit');
    }
    return res.json();
  }
  async updateUnit(id: string, data: any) {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error((err as any).error || 'Failed to update unit');
    }
    return res.json();
  }
  async deleteUnit(id: string) {
    const res = await fetch(`/api/units/${id}`, { method: 'DELETE', credentials: 'include' });
