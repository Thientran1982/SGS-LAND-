...
- **SEO & GEO Optimization**:
    ...
    - **Sprint #64 SEO/GEO upgrade (May 2026)**:
        ...
        - **Drift (deferred to follow-up tasks)**: 10 VI blog seed posts (DB-backed `/news` requires content + seed script), pillar Definition-Block rewrites, Lighthouse perf pass on 10 pillars, Event/AggregateRating/ImageObject geo schema additions.
    - **Sprint #64 follow-up — GEO Monitor (Task #66, May 2026)**:
        - New table `seo_geo_snapshots` (migration `102`): one row per day with `ai_mentions_json`, `gsc_top20_json`, `backlinks_json`, `lighthouse_json`. Unique on `date` so re-runs upsert.
        - `server/routes/geoMonitorCronRoutes.ts` — `POST /api/internal/geo-monitor-cron` (QStash, secret-protected) probes **Gemini / OpenAI / Anthropic / Perplexity / xAI Grok** with a 5-query brand prompt set, scores brand mentions (Perplexity also counts brand presence in returned `citations`), snapshots top-20 best SERP positions from `seo_target_keywords`, and probes 5 competitors (`batdongsan.com.vn`, `nhatot.com`, `cafeland.vn`, `kingsland.vn`, `cenland.vn`) for liveness, response latency, presence of an outbound link to `sgsland.vn` on their homepage, and (when `GOOGLE_CSE_KEY`/`GOOGLE_CSE_CX` are configured) Google Custom Search count of pages on the competitor domain co-mentioning our brand. Writes one row to `seo_geo_snapshots` keyed on the **ICT (Asia/Ho_Chi_Minh) calendar date** so the 21:30 UTC schedule (= 04:30 ICT next day) labels rows with the correct Vietnamese business day. Engines without API keys are recorded as `skipped` rather than failing. Aggregation filters `seo_target_keywords` by host tenant (`DEFAULT_TENANT_ID`) so other tenants' keyword data never leaks into the snapshot. Also exposes `GET /api/seo/geo-snapshots?days=30` and `POST /api/seo/geo-snapshots/run-now` — both gated to **host-tenant SUPER_ADMIN only** (consistent with other GEO/SEO management endpoints, since the snapshot holds competitive intelligence).
        - QStash schedule `geo-monitor-daily` (`30 21 * * *` UTC = 4:30 SA ICT) — 7th schedule total, well under maxSchedules=10.
        - SeoManager → GEO tab: new sub-section `1.5 GEO Monitor — 30 ngày qua` charts daily mention rate per engine (5 series) + best SERP position deltas for top-5 tracked keywords (Recharts), shows latest engine breakdown, last-snapshot delta, and a competitor backlinks/co-mention table, with a "Chạy snapshot ngay" button for manual triggering.
        - Optional env: `GEO_MONITOR_CRON_SECRET` (falls back to first 32 chars of `JWT_SECRET`); `GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `PERPLEXITY_API_KEY` / `XAI_API_KEY` enable per-engine probes; `GOOGLE_CSE_KEY` + `GOOGLE_CSE_CX` enable real co-mention counts in `backlinks_json`.
        - Lighthouse perf snapshot (`lighthouse_json`) is still a placeholder pending PSI/Lighthouse-CI wiring in a future sprint; schema is in place so it can land without migration.
