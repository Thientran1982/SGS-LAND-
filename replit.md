...
- **Public API for unauthenticated users**: `GET /api/public/listings` and `GET /api/public/listings/:id` are auth-free routes registered in `server.ts` before the `/api/listings` authenticated routes. They use `PUBLIC_TENANT = '00000000-0000-0000-0000-000000000001'` and only return `status = 'AVAILABLE'` listings. `dbApi.ts` exposes `getPublicListings()` and `getSimilarListings()` using these endpoints. `getListingById()` falls back to the public endpoint on 401. `Landing.tsx` and `ProductSearch.tsx` use `getPublicListings()` directly.
- **Seed script is not re-entrant**: `server/seed.ts` checks `SELECT COUNT(*) FROM users` — if users exist it skips entirely. If proposals/listings/leads are manually deleted, re-seeding is blocked. To restore: manually INSERT records using `executeSql`. Current DB state: 9 leads, 4 listings, 6 proposals (2 APPROVED), 1 contract (SIGNED), 25 interactions.
