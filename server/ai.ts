...
${favIds.size > 0 ? '5. Nếu có BĐS trùng watchlist: ghi chú "★ ĐÃ LƯU" và đề xuất phương án so sánh' : ''}`;
            ...
            const inventoryAI = await generateWithFallback({
                ...
                config: { systemInstruction: inventorySystemInstruction, ...taskProfile('SPECIALIST_STANDARD') }
            });
...
5. 1 gợi ý tối ưu hoá cụ thể phù hợp kịch bản này`;
            ...
            const financeAI = await generateWithFallback({
                ...
                config: { systemInstruction: financeSystemInstruction, ...taskProfile('SPECIALIST_STANDARD') }
            });
...
Viết tiếng Việt, bullet point, tối đa 180 từ. Tuyệt đối không trích dẫn điều khoản luật khô khan — dùng ngôn ngữ thực tế.`;
            ...
            const legalAI = await generateWithFallback({
                ...
                config: { systemInstruction: legalSystemInstruction, ...taskProfile('SPECIALIST_EXTENDED') }
            });
...
Viết tiếng Việt, bullet point, thực tế, tối đa 150 từ. Đây là ghi chú NỘI BỘ cho Sales — không phải tin nhắn trả lời khách.`;
            ...
            const bookingAI = await generateWithFallback({
                ...
                config: { systemInstruction: salesSystemInstruction, ...taskProfile('SPECIALIST_STANDARD') }
            });
...
Viết tiếng Việt, bullet point, thực tế, tối đa 160 từ.`;
            ...
            const marketingAI = await generateWithFallback({
                ...
                config: { systemInstruction: marketingSystemInstruction, ...taskProfile('SPECIALIST_STANDARD') }
            });
...
5. ${isRisk ? 'TOP 3 điều khoản bất lợi thường bị che giấu trong loại HĐ này' : 'Công chứng/thuế phí thực tế cần chuẩn bị (thuế TNCN 2%, lệ phí trước bạ 0.5%)'}`;
            ...
            const contractAI = await generateWithFallback({
                ...
                config: { systemInstruction: contractSystemInstruction, ...taskProfile('SPECIALIST_EXTENDED') }
            });
...
6. HÀNH ĐỘNG NGAY cho Sales (1 việc cụ thể làm hôm nay)`;
            ...
            const analysisRes = await generateWithFallback({
                ...
                config: {
                    ...
                    ...taskProfile('SPECIALIST_EXTENDED'),
                }
            });
