...
const CHAT_ICONS = {
    ...
    CHECK_SMALL: <svg className="w-2.5 h-2.5 text-sgs-verified" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" /></svg>
};
// Components
    ...
    return (
        <div className="mb-3 w-full max-w-full flex justify-end">
            ...
            >
                ...
                <svg className={`w-3 h-3 transition-transform duration-300 text-sgs-primary shrink-0 ${isExpanded ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
            </button>            
            {isExpanded && (
                <div className="mt-2 pl-2 space-y-2 animate-enter py-1 w-full">
                    {steps.map((step, idx) => {
                        ...
                        return (
                            <div key={idx} className="relative group pl-3 border-l border-sgs-border w-full">
                                <div className="flex items-start gap-2 w-full">
                                    <div className={`mt-0.5 w-3 h-3 rounded-full flex items-center justify-center shrink-0 border ${isRouter ? 'bg-[var(--glass-surface-hover)] border-[var(--glass-border)] text-[var(--text-tertiary)]' : 'bg-[var(--bg-surface)] border-sgs-primary/30 text-sgs-primary'}`}>
                                        {step.status === 'DONE' ? CHAT_ICONS.CHECK_SMALL : <div className="w-1.5 h-1.5 bg-sgs-on-dark-muted rounded-full animate-pulse" />}
                                    </div>
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                                            <span className={`text-xs2 font-bold uppercase tracking-tight ${isRouter ? 'text-[var(--text-tertiary)]' : 'text-sgs-primary'}`}>
        ...
        <div className="p-4">
            <div className="flex justify-between items-end mb-4">
                <div>
                    ...
                    <div className="text-xl font-extrabold text-sgs-primary">{formatCurrency(data.monthlyPayment)}</div>
                </div>
...
export const BookingWidget = memo(({ data, t, formatDateTime }: { data: BookingDraftData, t: any, formatDateTime: any }) => (
    <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--glass-border)] overflow-hidden shadow-sm my-2 max-w-sm w-full">
        <div className="bg-sgs-champagne px-4 py-3 border-b border-sgs-border flex items-center gap-2 min-w-0">
            <div className="text-sgs-primary shrink-0">{CHAT_ICONS.CALENDAR}</div>
            <h4 className="font-bold text-sgs-primary text-xs truncate">{t('inbox.booking_title')}</h4>
        </div>
        <div className="p-4 space-y-3">
            ...
            <button type="button" className="w-full py-2 bg-sgs-primary text-white font-bold rounded-lg text-xs hover:bg-sgs-primary transition-colors shadow-sm">
            ...
            {sources.groundingChunks.map((chunk, i) => {
                ...
                return (
                    <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="flex items-center gap-1.5 bg-[var(--glass-surface)] hover:bg-[var(--bg-surface)] border border-[var(--glass-border)] hover:border-sgs-primary rounded-full px-2.5 py-1 text-xs2 text-[var(--text-secondary)] transition-all max-w-full truncate group"
                    >
                        <span className="shrink-0">{CHAT_ICONS.LINK}</span>
                        <span className="truncate group-hover:text-sgs-primary">{title}</span>
                    </a>
                );
            })}
        </div>
...
export const AudioBubble = memo(({ duration }: { duration: number }) => {
    ...
    return (
        <div className="flex items-center gap-2 md:gap-3 min-w-[120px] w-full">
            <button aria-label={t('inbox.play_audio')} className="w-8 h-8 rounded-full bg-[var(--glass-surface-hover)] flex items-center justify-center text-[var(--text-secondary)] hover:bg-sgs-champagne hover:text-sgs-primary transition-colors shrink-0">
                {CHAT_ICONS.PLAY}
            </button>
...
export const FileBubble = memo(({ name, size, url }: { name: string, size?: number, url?: string }) => {
    ...
    return (
        <div className="flex items-center gap-2 md:gap-3 p-1 w-full">
            <div className="w-8 h-8 md:w-10 md:h-10 bg-sgs-champagne text-sgs-primary rounded-lg flex items-center justify-center shrink-0">
                {CHAT_ICONS.FILE}
            </div>
            <div className="min-w-0 flex-1">
